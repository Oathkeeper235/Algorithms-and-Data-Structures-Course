...
public class KumanovskiDijalekt {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < zborovi.length; i++) {
            ...
            char last = zborovi[i].charAt(zborovi[i].length() - 1);

            if (last == '.' || last == ',' || last == '!' || last == '?') {
                key = zborovi[i].substring(0, zborovi[i].length() - 1);
                znak = true;
            } else
                key = zborovi[i];
            ...
            if (cbht.search(key) != null) {
                String value = cbht.search(key).element.value;
