...
public class Paytolls {
    public static void main(String[] args) throws IOException {
        ...
        int cityA = Integer.parseInt(split[0]) - 1;
        int cityB = Integer.parseInt(split[1]) - 1;

        int numDirectPaths = Integer.parseInt(br.readLine());

        Integer[] edges = new Integer[numDirectPaths];
        for (int i = 0; i < numDirectPaths; i++) {
            edges[i] = i;
        }

        Graph<Integer> graph = new Graph<Integer>(numCities, edges);

        for (int i = 0; i < numDirectPaths; i++) {
            String[] splits = br.readLine().split("\\s+");
            graph.addEdge(Integer.parseInt(splits[0]) - 1, Integer.parseInt(splits[1]) - 1, Integer.parseInt(splits[2]));
            graph.addEdge(Integer.parseInt(splits[1]) - 1, Integer.parseInt(splits[0]) - 1, Integer.parseInt(splits[2]));
        }


        // Find path with lowest number of paytolls using Dijkstra algorithm
        System.out.println((int) graph.dijkstra(cityA)[cityB]);
