...
class FacebookProfile {
    ...
    public FacebookProfile(int id, int length) {
        this.id = id;
        this.length = length;
    }
...
public class CountFacebookFriends {
    public static void main(String[] args) throws IOException {
        ...
        FacebookProfile[] profiles = new FacebookProfile[N];
        boolean[] visited = new boolean[N];

        for (int i = 0; i < N; i++)
            profiles[i] = new FacebookProfile(i, 0);

        Graph<FacebookProfile> facebook = new Graph<FacebookProfile>(N, profiles);

        for (int i = 0; i < N; i++) {
            input = br.readLine();
            int brNaPrijateliNaKorisnik = Integer.parseInt(input);

            for (int j = 0; j < brNaPrijateliNaKorisnik; j++) {
                input = br.readLine();
                int id = Integer.parseInt(input.split("\\s+")[0]);

                facebook.addEdge(i, id);
                facebook.addEdge(id, i);
            }
        }
        ...
        Queue<FacebookProfile> queue = new LinkedQueue<>();
        queue.enqueue(profiles[x]);
        visited[x] = true;

        while (!queue.isEmpty()) {
            FacebookProfile profile = queue.dequeue();

            if (profile.getId() == y) {
                System.out.println(profile.getLength());
                break;
            }

            for (int i = 0; i < N; i++) {
                if (facebook.adjacent(profile.getId(), i) == 1 && !visited[i]) {
                    queue.enqueue(profiles[i]);
                    visited[i] = true;
                    profiles[i].setLength(profile.getLength() + 1);
                }
            }
        }
    }
